...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/jacquessinger/Documents/Academics/Research/BCI Projects/MIL_PsychoPy_fMRI/purpose/purpose_task_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instruct = visual.TextBox2(
         ...
         ori=0.0, pos=(0, 0), draggable=True,      letterHeight=0.05,
    ...
    task_verification_message = visual.TextStim(win=win, name='task_verification_message',
        text='Were you able to reflect on this life purpose?\n\nPress 1 for yes and 2 for no. ',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    for thisTrial_2 in trials_2:
        ...
        while continueRoutine:
            ...
            if instruct.status == STARTED:
                # update params
                instruct.setText(purpose_instructions, log=False)
    ...
    for thisTrial in trials:
        ...
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            # *purp_textbox* updates
            
            # if purp_textbox is starting this frame...
            if purp_textbox.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                purp_textbox.frameNStart = frameN  # exact frame index
                purp_textbox.tStart = t  # local t and not account for scr refresh
                purp_textbox.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(purp_textbox, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'purp_textbox.started')
                # update status
                purp_textbox.status = STARTED
                purp_textbox.setAutoDraw(True)
            
            # if purp_textbox is active this frame...
            if purp_textbox.status == STARTED:
                # update params
                purp_textbox.setText(purpose_text_01
                , log=False)
            
            # if purp_textbox is stopping this frame...
            if purp_textbox.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > purp_textbox.tStartRefresh + 10-frameTolerance:
                    # keep track of stop time/frame for later
                    purp_textbox.tStop = t  # not accounting for scr refresh
                    purp_textbox.tStopRefresh = tThisFlipGlobal  # on global time
                    purp_textbox.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'purp_textbox.stopped')
                    # update status
                    purp_textbox.status = FINISHED
                    purp_textbox.setAutoDraw(False)
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if textbox_task.status == STARTED:
                # update params
                textbox_task.setText(purpose_text_01, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if textbox.status == STARTED:
                # update params
                textbox.setText(purpose_text_01, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            if reading2_textbox.status == STARTED:
                # update params
                reading2_textbox.setText(purpose_text_02, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if control2_textbox.status == STARTED:
                # update params
                control2_textbox.setText(purpose_text_02, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if task2_textbox.status == STARTED:
                # update params
                task2_textbox.setText(purpose_text_02, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 5.0:
            ...
            if reading3_textbox.status == STARTED:
                # update params
                reading3_textbox.setText(purpose_text_03, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if task3_textbox.status == STARTED:
                # update params
                task3_textbox.setText(purpose_text_03, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if control3_textbox.status == STARTED:
                # update params
                control3_textbox.setText(purpose_text_03, log=False)
