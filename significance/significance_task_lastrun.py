...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/jacquessinger/Documents/Academics/Research/BCI Projects/MIL_PsychoPy_fMRI/significance/significance_task_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    textbox_significance = visual.TextBox2(
         win, text='', placeholder='Type here...', font='Arial',
         ori=0.0, pos=(0, 0), draggable=False,      letterHeight=0.05,
    ...
    task_verification_message = visual.TextStim(win=win, name='task_verification_message',
        text='Were you able to start reflecting on your significance to this person?\n\nPress 1 for yes and 2 for no. ',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    for thisTrial_2 in trials_2:
        ...
        while continueRoutine:
            ...
            if instruct.status == STARTED:
                # update params
                instruct.setText(significance_instructions, log=False)
    ...
    for thisTrial in trials:
        ...
        reading = data.Routine(
            name='reading',
            components=[textbox_significance],
        )
        reading.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        textbox_significance.reset()
        ...
        while continueRoutine and routineTimer.getTime() < 5.0:
            ...
            # *textbox_significance* updates
            
            # if textbox_significance is starting this frame...
            if textbox_significance.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                textbox_significance.frameNStart = frameN  # exact frame index
                textbox_significance.tStart = t  # local t and not account for scr refresh
                textbox_significance.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(textbox_significance, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'textbox_significance.started')
                # update status
                textbox_significance.status = STARTED
                textbox_significance.setAutoDraw(True)
            
            # if textbox_significance is active this frame...
            if textbox_significance.status == STARTED:
                # update params
                textbox_significance.setText(significance_text_01
                , log=False)
            
            # if textbox_significance is stopping this frame...
            if textbox_significance.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > textbox_significance.tStartRefresh + 5-frameTolerance:
                    # keep track of stop time/frame for later
                    textbox_significance.tStop = t  # not accounting for scr refresh
                    textbox_significance.tStopRefresh = tThisFlipGlobal  # on global time
                    textbox_significance.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'textbox_significance.stopped')
                    # update status
                    textbox_significance.status = FINISHED
                    textbox_significance.setAutoDraw(False)
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if textbox_task.status == STARTED:
                # update params
                textbox_task.setText(significance_text_01, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if textbox.status == STARTED:
                # update params
                textbox.setText(significance_text_01
                , log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 10.0:
            ...
            if reading2_textbox.status == STARTED:
                # update params
                reading2_textbox.setText(significance_text_02, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if control2_textbox.status == STARTED:
                # update params
                control2_textbox.setText(significance_text_02, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if task2_textbox.status == STARTED:
                # update params
                task2_textbox.setText(significance_text_02, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 5.0:
            ...
            if reading3_textbox.status == STARTED:
                # update params
                reading3_textbox.setText(significance_text_03, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if task3_textbox.status == STARTED:
                # update params
                task3_textbox.setText(significance_text_03, log=False)
        ...
        while continueRoutine and routineTimer.getTime() < 20.0:
            ...
            if control3_textbox.status == STARTED:
                # update params
                control3_textbox.setText(significance_text_03, log=False)
